...
public class Player {
	...
	public String allCardsNames = "";
	public String allStocksNames = "";
			...
			if(reverse){ //even dice, change track if possible
				...
				for(int i=0;i<dice;i++){
					...
					if(s.type.equals("Transit")){
						...
						s = board.nextSquareForSetPosition(posId);
						posId = s.id;
					}
				}
		...
		while(true){
			...
			if(s.type.equals("Property")){
				...
				if(temp.owner==null){
					isFound = true;
					break;
					// s'in pozisyonuna gidecek.
				}else if(temp.owner.id != this.id && !firstOwnedProperty){
	...
	public void setFreeProperties(){
		...
		for(int i=0;i<6;i++){
			if(shares[i]>0){
				allStocksNames += board.bank.names[i]+" ("+shares[i]+"/6) ";
				if(abc!=0)
					allStocksNames += ", ";
				abc++;
			}	
